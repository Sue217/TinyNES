...
void PPU::step() {
  switch (m_pipelineState) {
    case PreRender:
      ...
      } else if (280 < m_cycle && m_cycle <= 304 && m_showBackground && m_showSprites) {
        // Set vertical bits
    ...
    case Render:
      if (m_cycle > 0 && m_cycle <= ScanlineVisibleDots) {
        ...
        if (m_showBackground) {
          ...
          if (!m_hideEdgeBackground || x >= 8) {
            ...
            // Get the corresponding bit determined by (8 - x_fine) from the
            // right
        ...
        if (m_showSprites && (!m_hideEdgeSprites || x >= 8)) {
          for (auto i : m_scanlineSprites) {
            Data spr_x = m_spriteMemory[i * 4 + 3];

            if (0 > x - spr_x || x - spr_x >= 8) continue;

            Data spr_y = m_spriteMemory[i * 4 + 0] + 1,
      ...
      if (m_cycle >= ScanlineEndCycle) {
        ...
        int range = 8;
        if (m_longSprites) range = 16;

        for (std::size_t i = m_spriteDataAddress / 4, j = 0; i < 64; ++i) {
      ...
      if (m_scanline >= VisibleScanlines) m_pipelineState = PostRender;
    ...
    case PostRender:
      if (m_cycle >= ScanlineEndCycle) {
        ...
        for (int x = 0; x < (int) m_pictureBuffer.size(); ++x) {
          for (int y = 0; y < (int) m_pictureBuffer[0].size(); ++y) {
...
Data PPU::readOAM(Data addr) { return m_spriteMemory[addr]; }

void PPU::writeOAM(Data addr, Data value) { m_spriteMemory[addr] = value; }

void PPU::doDMA(const Data* page_ptr) {
...
Data PPU::getStatus() {
  Data status = m_sprZeroHit << 6 | m_vblank << 7;
  // m_dataAddress = 0;
  m_vblank = false;
  m_firstWrite = true;
  return status;
...
void PPU::setDataAddress(Data addr) {
...
Data PPU::getOAMData() { return readOAM(m_spriteDataAddress); }

void PPU::setData(Data data) {
  m_bus.write(m_dataAddress, data);
  m_dataAddress += m_dataAddrIncrement;
}

void PPU::setOAMAddress(Data addr) { m_spriteDataAddress = addr; }

void PPU::setOAMData(Data value) { writeOAM(m_spriteDataAddress++, value); }

void PPU::setScroll(Data scroll) {
  if (m_firstWrite) {
    m_tempAddress &= ~0x1f;
...
Data PPU::read(Address addr) { return m_bus.read(addr); }
